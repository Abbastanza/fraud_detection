...
model_vars <- c("type", "amount", "oldbalanceOrg", "newbalanceOrig", "change.balanceOrg", "flagFraud", "isFraud")
...
```{r}
#Remove nameDestType from the data
trainData$nameDestType <- NULL
valData$nameDestType <- NULL
testData$nameDestType <- NULL
```
...
```{r}
# RF
rf_model_full <- ranger(isFraud ~ ., data = fullTrainData, importance = 'impurity')
# Predict on the test data
...
saveRDS(rf_model_full, "rf_model_full.rds")
```
