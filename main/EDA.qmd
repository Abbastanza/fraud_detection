...
Wir haben schon bei den EDA festgestellt dass die bei uns feature engineerte variable flagFraud schon ganz gut Fraud Fälle finden kann. Deswegen haben wir unsere modelle dazu verwendet um die ergebnisse noch zu verbessern. Wir haben dazu drei modelle gewählt die mit so unbalancierte Daten umgehen können: Decision Tree, SVM, Random Forest. Um den modellen eficienter trenieren zu können haben wir nur an den daten treniert wo den transaktionstyp cash_out oder transfer war.

### Decision Tree

```{r}
# Stratified allocation based on the combined column
splitIndex <- createDataPartition(filtered_df$isFraud, p = 0.75, list = FALSE, times = 1)

trainData <- filtered_df[splitIndex, ]
testData <- filtered_df[-splitIndex, ]
```
...
```{r}
# Train the model
rf_model <- ranger(isFraud ~ ., data = trainData2, ntree = 300, importance = 'impurity')

# Summarize the model
print(summary(rf_model))

# Predict on the test set
predictions <- predict(rf_model, testData2)
```

```{r}
predictions_vector <- predictions$predictions  # Extract predictions

# Assess the model's performance
actual <- testData2$isFraud
confusion_matrix <- table(Predicted = predictions_vector, Actual = actual)
